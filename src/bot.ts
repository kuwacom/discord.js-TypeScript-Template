...
(async () => {
  ...
  const commandFiles = fs
    .readdirSync(TSDistPath + '/commands')
...
(async () => {
  ...
  const buttonFiles = fs
    .readdirSync(TSDistPath + '/buttons')
    .filter((file) => file.endsWith('.js'));
...
(async () => {
  ...
  const selectMenuFiles = fs
    .readdirSync(TSDistPath + '/selectMenus')
    .filter((file) => file.endsWith('.js'));
...
async function statusTask() {
  ...
  while (1) {
    ...
    client.user?.setPresence({
      activities: [
        { name: `${guildSize} サーバー`, type: Discord.ActivityType.Competing },
      ],
    });
    await sleep(10000);
    client.user?.setPresence({
